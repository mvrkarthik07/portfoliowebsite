...
/**
 * Re-run Unicorn Studio init after SPA navigation: the library only auto-inits once globally,
 * but our [data-us-project] node is recreated when returning to Home.
 */
function unicornReinit() {
  ...
  try {
    ...
    /* library may throw if DOM not ready; next interval tick will retry */
  }
}
...
const BRANDING_CSS = `
  ...
  /* Vitruvian-only hero: kill flat “panel” behind the figure, soften hard edges */
  .hero-ascii-embed-root {
    isolation: isolate;
    -webkit-mask-image: radial-gradient(
      ellipse 72% 88% at 74% 46%,
      #000 0%,
      #000 52%,
      transparent 88%
    );
  ...
  .hero-ascii-embed-root [data-us-project] {
  ...
  .hero-ascii-embed-root [data-us-project] canvas {
    clip-path: inset(2% 4% 22% 4%) !important;
    background-color: transparent !important;
  }
`
...
export interface HeroAsciiProps {
  /** Unicorn Studio project id (embed) */
  unicornProjectId?: string
  /**
   * When true (default on Home), hides the duplicate top/bottom chrome so your site Navbar/Footer stay primary.
   */
  embedded?: boolean
  brandTitle?: string
  ...
  /** Optional slot below buttons (e.g. scroll hint) */
  children?: ReactNode
  /** Only the Unicorn Vitruvian embed (+ mobile stars); no headline, CTAs, or children UI */
  asciiOnly?: boolean
}

export default function HeroAscii({
  unicornProjectId = 'whwOGlfJ5Rz2rHaEUgHl',
  embedded = true,
  brandTitle = 'UIMIX',
  brandSubtitle = 'EST. 2025',
  titleLine1 = 'PERFECT',
  titleLine2 = 'PROPORTIONS',
  description = "Where geometry meets humanity — Da Vinci's vision of ideal form",
  primaryCta = { label: 'GET STARTED', to: '/work' },
  secondaryCta = { label: 'LEARN MORE', to: '/posters' },
  children,
  asciiOnly = false,
}: HeroAsciiProps) {
  ...
  useEffect(() => {
    ...
    style.setAttribute('data-hero-ascii', '1')
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return
        // Let React commit the [data-us-project] node before init scans the DOM
        requestAnimationFrame(() => {
          requestAnimationFrame(tick)
        })
        // SPA remount can race WebGL; retry a few times after navigation back to Home
        const retryMs = [50, 200, 500, 1200]
        retryMs.forEach((ms) => {
          retryIds.push(
            window.setTimeout(() => {
              if (!cancelled) tick()
    ...
    hideBranding()
    const interval = window.setInterval(hideBranding, 100)
    const t1 = window.setTimeout(hideBranding, 1000)
    const t2 = window.setTimeout(hideBranding, 3000)
    const t3 = window.setTimeout(hideBranding, 5000)
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-transparent">
      <div
        ref={embedRootRef}
        className={cn(
          'absolute inset-0 h-full min-h-[100svh] w-full',
          asciiOnly ? 'hero-ascii-embed-root z-[1] block' : 'hidden lg:block',
        )}
        aria-hidden
      >
        <div
      ...
      <div
        className={cn(
          'hero-ascii-stars-bg absolute inset-0 h-full w-full',
          asciiOnly ? 'hidden' : 'lg:hidden',
        )}
      />
      ...
      {!embedded && (
        <>
          ...
          <div
            className="absolute bottom-[5vh] left-0 z-20 h-8 w-8 border-b-2 border-l-2 border-white/30 lg:h-12 lg:w-12"
          />
          <div
            className="absolute bottom-[5vh] right-0 z-20 h-8 w-8 border-b-2 border-r-2 border-white/30 lg:h-12 lg:w-12"
          />
        </>
      )}
      ...
      {!asciiOnly && (
        <div
          className={`relative z-10 flex min-h-screen items-center ${embedded ? 'pt-24 md:pt-28' : 'pt-16 lg:pt-0'}`}
          style={{ marginTop: embedded ? undefined : '5vh' }}
        >
          <div className={`container mx-auto px-6 lg:px-16 ${embedded ? '' : 'lg:ml-[10%]'}`}>
            <div className="relative max-w-lg">
              ...
              <div className="relative">
                <div className="hero-ascii-dither absolute -left-3 top-0 bottom-0 hidden w-1 opacity-40 lg:block" />
                <h1
                  className="mb-3 font-mono text-2xl font-bold tracking-wider text-white lg:mb-4 lg:text-5xl"
              ...
              <div className="relative">
                ...
                <div
                  className="absolute -right-4 top-1/2 hidden h-3 w-3 border border-white opacity-30 lg:block"
              ...
              {!embedded && (
                <div className="mt-6 hidden items-center gap-2 opacity-40 lg:flex">
                  ...
                  <div className="h-px flex-1 bg-white" />
                  <span className="font-mono text-[9px] text-white">VITRUVIAN</span>
                </div>
              )}
            </div>
          </div>
