...
/**
 * Full-viewport WebGL neural vortex (pointer + scroll reactive). Canvas only — no inner chrome.
 */
export function InteractiveNeuralVortexBackground({
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        color = vec3(0.55, 0.06, 0.1);
        color = mix(color, vec3(0.95, 0.18, 0.12), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.42, 0.02, 0.04) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn(
        'pointer-events-none fixed inset-0 z-0 h-full min-h-[100dvh] w-full opacity-95',
        className,
      )}
      aria-hidden
    />
